...
function ProductModal({ product, categories, onClose, onRefresh }) {
  ...
  // 'items' = a normal (non-weighed) product that gets a SHORT 6-digit barcode
  // generated by the "Create Barcode" button on the Store page.
  const UNITS = ['unit', 'items', 'kg', 'liter', 'meter', 'g', 'ml', 'pcs']
  ...
  // ── SHORT BARCODES ──
  // Ask the backend for the next free 6-digit codes (200001+). Short so they
  // print small on a label, and guaranteed not to clash with existing barcodes
  // or the Kg scale PLU range. (Was: 'POS' + Date.now() — 19 chars, far too wide.)
  ...
  const handleCreateBarcodes = async () => {
    ...
    if (r.success) {
      setPluMsg(r.generated > 0
        ? `✅ Created ${r.generated} short barcode${r.generated === 1 ? '' : 's'} (Kg scale items + 'items' unit products).`
        ...
        <div className="flex-between" style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button
              ...
              title="Generate short 6-digit barcodes: Kg items get scale PLU codes, 'items' unit products get 200000+ codes"
            >
