...
export default function Billing() {
  ...
  const loadQuickSale = async () => {
    // If preload.js wasn't reloaded, these APIs won't exist. Fail loudly.
    if (!window.api?.getQuickSale) {
      console.error('Quick Sale API missing — preload.js not loaded. Restart Electron.')
      return
    }
    try {
      const r = await window.api.getQuickSale()
      if (r.success) setQuickSale(r.data)
      else console.error('getQuickSale failed:', r.message)
    } catch (err) {
      console.error('getQuickSale error:', err)
    }
  }

  // Pin a searched product to Quick Sale (⚡ button or right-click)
  const handleAddToQuickSale = async (row) => {
    if (!window.api?.addQuickSale) {
      setErrors(['Quick Sale is not available. Close the app completely and restart it.'])
      setTimeout(() => setErrors([]), 5000)
      return
    }
    if (!row?.variant_id) {
      setErrors(['Could not read the product variant.'])
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        style={{
                          ...styles.pinBtn,
                          ...(pinned ? styles.pinBtnOn : {})
                        }}
                        title={pinned ? 'Already in Quick Sale' : 'Add to Quick Sale'}
                        disabled={pinned}
                        onClick={(e) => {
                          e.stopPropagation()   // don't open the product panel
                          handleAddToQuickSale(row)
                        }}
                      >
                        {pinned ? '✓ Pinned' : '⚡ Pin'}
...
const styles = {
  ...
  dropdown: {
    ...
    zIndex: 9999,            // must sit above the Quick Sale card below it
  ...
  qsVariant: { fontSize: '11px', color: '#6b7280' },
  qsPrice: { fontWeight: '800', fontSize: '16px', marginTop: '2px' },
