  ...
  const handleSaveBill = async (directPrint = false, skipConfirm = false) => {
    ...
    if (result.success) {
      if (directPrint) {
        ...
        if (s.success) printBillHTML(result, s.data, cart, customerName, grandTotal, totalDiscount, parseFloat(cashPaid), change)
      }
      setCart([])
      setCustomerName('')
      setCashPaid('')
...
function printBillHTML(billData, settings, cart, customerName, grandTotal, totalDiscount, cashPaid, change) {
  const items = cart.map(item => `
    <tr>
      <td>${item.productName} - ${item.variantName}${item.isPriceEdited ? ' ✏️' : ''}</td>
  ...
  const html = `<!DOCTYPE html><html><head><title>Bill ${billData.billNumber}</title>
    <style>body{font-family:monospace;width:80mm;margin:0 auto;font-size:12px;}
    h2,p{text-align:center;margin:2px 0;}table{width:100%;border-collapse:collapse;}
    th,td{padding:3px 2px;font-size:11px;}th{border-bottom:1px dashed #000;}
    .t{border-top:1px dashed #000;font-weight:bold;}@media print{body{margin:0;}}</style>
    </head><body>
    ...
    ${customerName ? `<p>Customer:${customerName}</p>` : ''}<hr/>
    <table><thead><tr><th>Item</th><th>Qty</th><th>Price</th><th>Total</th></tr></thead>
    <tbody>${items}</tbody></table><hr/>
    <table>
    ${totalDiscount > 0 ? `<tr><td>Discount:</td><td style="text-align:right">Rs.${totalDiscount.toFixed(2)}</td></tr>` : ''}
    <tr class="t"><td>TOTAL:</td><td style="text-align:right">Rs.${grandTotal.toFixed(2)}</td></tr>
    <tr><td>Cash:</td><td style="text-align:right">Rs.${cashPaid.toFixed(2)}</td></tr>
    <tr><td>Change:</td><td style="text-align:right">Rs.${change.toFixed(2)}</td></tr>
    </table><hr/><p>${settings.bill_thank_you || 'Thank you!'}</p>
